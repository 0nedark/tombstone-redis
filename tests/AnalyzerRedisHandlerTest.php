...
use OneDark\TombstoneRedis\AnalyzerRedisHandler;
use Redis;
...
class AnalyzerRedisHandlerTest extends TestCase
{
    ...
    {
        $handler = new AnalyzerRedisHandler(self::$redis);
        $handler->log(Fixture::getVampire(...TestCase::TOMBSTONE_ARGUMENTS));

        $logFiles = $this->getLogFiles();
        $this->assertCount(1, $logFiles);

        $logFileContent = $this->readLogFiles($logFiles);
        $this->assertEquals(TestCase::LOG_RECORD.PHP_EOL, $logFileContent);
    }
}
